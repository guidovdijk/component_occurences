 ...
 */
const overrideCustomOptions = (obj, newValues) => {
  for (const [key, value] of Object.entries(newValues)) {
    if (value !== undefined) {
      if (typeof value === "object" && !Array.isArray(value)) {
        obj[key] = overrideCustomOptions(obj[key] || {}, value);
...
const setOptions = () => {
  try {
    if(!fs.existsSync(optionsFilePath)){
      throw new Error(`${FILE_NAME} file does not exist in the root of the ${PROJECT_PATH} working directory`)
    }

    const optionsFileContent = JSON.parse(fs.readFileSync(optionsFilePath, 'utf8'))

    let newOptions = overrideCustomOptions(baseOptions, optionsFileContent)
